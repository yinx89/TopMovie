...
for j = 1:num_users
    for k = 1:num_features
        Theta_grad(j,k) = Error(:,j)'*X(:,k)+lambda*Theta(j,k);
    end
end
...
% AÑADO ESTO PARA APARTADO 2.4 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% En X_grad(i,k) añadir "+lambda*X(i,k)"
% En Theta_grad(j,k) añadir "+lambda*Theta(j,k)"
% En X_grad añadir "+lambda*X"
% En Theta_grad añadir "+lambda*Theta"
